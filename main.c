...
#define SERVER_ADDRESS          "47.93.253.168"//"123.206.23.115"//"47.101.60.213"//"m14.cloudmqtt.com"//"123.206.23.115"//
...
#define PUB_TOPIC       "/CloudAquarium2/send"

/*Defining Number of topics 定义主题数量*/
#define TOPIC_COUNT             1

/*Defining Subscription Topic Values 定义订阅主题值*/
#define TOPIC1                  "/CloudAquarium2/receive"

/*定义控制数据*/
#define DateHeatingOn				"\"heating\":\"1\""
#define DateHeatingOff				"\"heating\":\"0\""
#define DateOxygenOn				"\"oxygen\":\"1\""
#define DateOxygenOff				"\"oxygen\":\"0\""
#define DateChangeOn				"\"change\":\"1\""
#define DateChangeOff				"\"change\":\"0\""
#define DateCoolingOn				"\"cooling\":\"1\""
#define DateCoolingOff				"\"cooling\":\"0\""
#define DateFeedOn					"\"feed\":\"1\""
#define DateFeedOff					"\"feed\":\"0\""
#define DateFiltrationOn			"\"filtration\":\"1\""
#define DateFiltrationOff			"\"filtration\":\"0\""
...
//char *data_sw2={"{\"temp\":\"00.00\",\"pH\":\"00.00\",\"LiquidLevel\":\"0\"}"};
                       ...
                       long pay_len, bool dup,unsigned char qos, bool retain)
{
    ...
    *pay1 = '\0';
    ...
    UART_PRINT("\n\rdate3: %s\n\r",dev);
    *pay1 = c;
    if(strcmp(dev,DateChangeOn) == 0)
        {
        	MAP_GPIOPinWrite(GPIOA2_BASE,0x40,0);
        	UART_PRINT("change condition is: on\n\r");
        }
    else if(strcmp(dev,DateChangeOff) == 0)
    {
    	MAP_GPIOPinWrite(GPIOA2_BASE,0x40,0x40);
    	UART_PRINT("change condition is: off\n\r");
    }
    free(dev);
    ...
    *pay1 = '\0';
    ...
    UART_PRINT("\n\rdate4: %s\n\r",dev);
    *pay1 = c;
    if(strcmp(dev,DateCoolingOn) == 0)
        {
        	MAP_GPIOPinWrite(GPIOA1_BASE,0x08,0);
        	UART_PRINT("cooling condition is: on\n\r");
        }
    else if(strcmp(dev,DateCoolingOff) == 0)
    {
    	MAP_GPIOPinWrite(GPIOA1_BASE,0x08,0x08);
    	UART_PRINT("cooling condition is: off\n\r");
    }
    free(dev);
    ...
    *pay1 = '\0';
    ...
    UART_PRINT("\n\rdate5: %s\n\r",dev);
    *pay1 = c;
    if(strcmp(dev,DateFeedOn) == 0)
        {
    		MAP_GPIOPinWrite(GPIOA3_BASE,0x10,0);//**
        	//MAP_GPIOPinWrite(GPIOA1_BASE,0x01,0);
        	osi_Sleep(5300);
        	MAP_GPIOPinWrite(GPIOA3_BASE,0x10,0x10);//**
        	//do
        	//	;
        	//while(MAP_GPIOPinRead(GPIOA3_BASE,0x10) == 0);
        	//{
        	//	UART_PRINT("PIN_18 = 1");
        	//	MAP_GPIOPinWrite(GPIOA1_BASE,0x01,0x01);
        	//}

        	UART_PRINT("feed condition is: on\n\r");
        }
    else if(strcmp(dev,DateFeedOff) == 0)
    //{
    	MAP_GPIOPinWrite(GPIOA3_BASE,0x10,0x10);//**
    	//MAP_GPIOPinWrite(GPIOA1_BASE,0x01,0x01);
    	UART_PRINT("feed condition is: off\n\r");
    //}
    free(dev);
    ...
    *pay1 = '\0';
    ...
    UART_PRINT("\n\rdate 6: %s\n\r",dev);
    *pay1 = c;
    if(strcmp(dev,DateFiltrationOn) == 0)
        {
        	MAP_GPIOPinWrite(GPIOA2_BASE,0x01,0);
        	UART_PRINT("filtration condition is: on\n\r");
        }
    else if(strcmp(dev,DateFiltrationOff) == 0)
    {
    	MAP_GPIOPinWrite(GPIOA2_BASE,0x01,0x01);
    	UART_PRINT("filtration condition is: off\n\r");
    }
    free(dev);
...
connect_to_broker:
    ...
    for(;;)
    {
        ...
        if(PUSH_BUTTON_SW3_PRESSED == RecvQue.event)
        {
            ...
            float ph = 07.33;
            //float ph = ReadTemp();//float ph = ReadPh();
            int ph1 = (int)ph;
            int ph2 = (int)((int)(100*ph)%100);
            itoa(ph1,data_sw3+23);
            itoa(ph2, data_sw3+25);
            itoa(LiquidLevel,data_sw3+44);
            *(data_sw3+11) = '.';
